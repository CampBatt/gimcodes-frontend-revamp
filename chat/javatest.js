...
function clientCommand(command,timeID){
    if (! commands_ran.includes(timeID)){
        if (command.charAt(0) == '/'){
            ...
            if (parts[0]=='/finds'){
                if (parts[1] == user){
                    sendMessage(localStorage.getItem('finds'))
                    commands_ran.push(timeID)
                }
            }
            ...
            if (parts[0]=='/scanned'){
                if (parts[1] == user){
                    sendMessage(localStorage.getItem('scanned'))
                    commands_ran.push(timeID)
                }
            }
            if (parts[0]=='/scan'){
                if (parts[1] == user){
                    alert('running the scanner')
                    main_loop();
                    commands_ran.push(timeID)
                }
            }
...
if (localStorage.getItem("kicks") == null){
    localStorage.setItem("kicks",JSON.stringify(["abc"]))
}

function kick(user,kickID){
	var kicks = JSON.parse(localStorage.getItem('kicks'))
    if(localStorage.getItem('user') == user && !kicks.includes(kickID)){
	kicks.push(kickID)
	localStorage.setItem('kicks',JSON.stringify(kicks))
        setInterval(function(){location.href = 'about:blank'},3000)
    }
}
...
function sendMessage(message){
    if (message.charAt(0) == '/'){
        ...
        //}
        if (parts[0]=='/progress'){
            statuss = localStorage.getItem('bar');
            if (statuss == 'true'){
                localStorage.setItem('bar','false');
                outer_bar.className = 'notSeeable'
            }else{
    ...
    fetch(socialAPI,{
        ...
        body:JSON.stringify({
            ...
            if (messages != null){
                raw_refresh(messages,sessionStorage.getItem("time_id"));
            };
        }
            )
};
...
function getMessageState(){
    fetch(socialAPI).then(function(response){ //https://gimcodes-3eqojmtp2-campbatt.vercel.app/
            ...
            var messages = new_messages['new_messages_state']
            if (messages != null){
                testing_add_message_block(messages)
            }
        })
}

function update_message_state(){
    fetch(socialAPI + '/one').then(function(response){
        return response.json();
    }).then(function(new_message){
        temp_message = document.createElement("li")
        temp_message.innerHTML = '<ul><canvas width="35px" height="35px"></canvas><ul><li><ul><li><strong>'+new_message[1]+"#"+new_message[4]+'</strong></li><li class="time">'+TimeCalc(new_message[2])+'</li></ul></li><li class="messagething">'+new_message[0]+'</li></ul></ul>'
        drawProfilePicture(JSON.parse(new_message[3]),temp_message.firstChild.firstChild,new_message[4])
        scrollchat.appendChild(temp_message)
        setTimeout(()=>{scrollchat.scrollTop = scrollchat.scrollHeight- scrollchat.clientHeight}, 100)
        
    })
}

function refresh_MessageState(last_time_ID){
    //fetch new message state
    fetch(socialAPI).then(function(response){
        return response.json();
    }).then(function (new_messages){
        //console.log(new_messages);
        var messages = new_messages['new_messages_state']
        if (messages != null){
            //raw_refresh(messages,last_time_ID);
            testing_add_message_block(messages)
        }
    })
...
function raw_refresh(messages,last_time_ID){

    //var last_time_ID = parseInt(localStorage.getItem("time_id"))
    var current_message = first_message
    var needs_to_scroll = false
    //make messages invisible, like chris's father
    ...
    //add new messages
    for (let x = 0; x<messages.length; x++){
        if(parseInt(messages[x][2])>last_time_ID){
            //remove first message
            var temp_first_message = first_message.nextElementSibling
            scrollchat.removeChild(first_message)
            temp_first_message.id = "first_message"
            first_message = temp_first_message

            //appened new message
            var temp_new_message = document.createElement("li")
            temp_new_message.innerHTML = '<ul><canvas width="35px" height="35px"></canvas><ul><li><ul><li><strong>'+messages[x][1]+"#"+messages[x][4]+'</strong></li><li class="time">'+TimeCalc(messages[x][2])+'</li></ul></li><li class="messagething">'+messages[x][0]+'</li></ul></ul>'
            drawProfilePicture(JSON.parse(messages[x][3]),temp_new_message.firstElementChild.firstElementChild,messages[x][4])
            scrollchat.appendChild(temp_new_message)
            last_message = temp_new_message

            //changes vars
            last_time_ID = messages[x][2]
            needs_to_scroll = true
        }
        
    }
    //save time id for next call of the function
    sessionStorage.setItem("time_id",last_time_ID)
    console.log(first_message,"<--first message");
    //refresh the values of each message element
    var current_message = first_message
    for (let x = 0; x<messages.length; x++){
        ...
        Formater.innerHTML = messages[x][0];
        var temp_chat = current_message.firstElementChild.firstElementChild.nextElementSibling.firstElementChild.nextElementSibling;// = messages[x][0]
        var formated_message = Formater.innerHTML;
        var to_be_tested= temp_chat.innerHTML
        //console.log(to_be_tested.length,formated_message.length)
        if (to_be_tested != formated_message){
            temp_chat.innerHTML = formated_message
            //console.log(to_be_tested,formated_message)
        }
        current_message.firstElementChild.firstElementChild.nextElementSibling.firstElementChild.firstElementChild.firstElementChild.innerHTML = messages[x][1]+ '#'+messages[x][4]
        current_message.firstElementChild.firstElementChild.nextElementSibling.firstElementChild.firstElementChild.firstElementChild.nextElementSibling.innerHTML = TimeCalc(messages[x][2])
        drawProfilePicture(JSON.parse(messages[x][3]),current_message.firstElementChild.firstElementChild,messages[x][4])
        current_message = current_message.nextElementSibling
        };

    //scroll chat to the bottom if neccesary
    if (FirstTime || needs_to_scroll){
        FirstTime = false
...
//setInterval(refresh_MessageState,10000)
//setInterval(function(){refresh_MessageState(sessionStorage.getItem("time_id"))},10000)
